...
\newcommand{\feUniqueFixed}{43}
\newcommand{\feUniqueConfirmed}{6}
\newcommand{\feUniqueFixedOrConfirmed}{49}
\newcommand{\feValidDuplicates}{6}

%% Zig %%
\newcommand{\zigUniqueConfirmed}{1}
\newcommand{\zigUniqueFixed}{1}
\newcommand{\zigUniqueFixedOrConfirmed}{2}

%% ALL %%
\newcommand{\allValidDuplicates}{15} % \feValidDuplicates (6) + \solAValidDuplicates (9)
\newcommand{\allUniqueFixed}{125} % \solUniqueFixed (68) + \movUniqueFixed (12) + \zigUniqueFixed (1) + \feUniqueFixed (41)
\newcommand{\allUniqueConfirmed}{11} % \solUniqueConfirmed (2) + \movUniqueConfirmed (2) + \zigConfirmed (1) + \feUniqueConfirmed (8)
...
\section{Appendix A: Artifact Submission}

The full artifact information for this paper is available at
\url{https://zenodo.org/record/5982794} (DOI 10.5281/zenodo.5982794).  The zenodo link provides

\begin{enumerate}
\item The actual tool build used in  our evaluation.
\item Scripts and utilities for the experiments.
  \item A Docker image with the artifact set up to run on a
    representative compiler subject, {\tt solc}.
  \item Raw results of our fuzzing runs.
    \item More detailed documentation of the experimental methods and
      build process for the above.
    \end{enumerate}

    Please see {\tt README.md} at the Zenodo link for further details.

The latest version of the AFL compiler-fuzzer can always be found at \url{https://github.com/agroce/afl-compiler-fuzzer}.
